...
---
> [!WARNING]  
> - The SKR MINI E3 board as well as all other BTT boards don't support the proprietary printer screen and therefore need an RPi or another single-board computer capable of running Klipper for an online connection.
...
> - I used [this YouTube video](https://www.youtube.com/watch?v=VxCBDyvFSgA) for the pinout and advise you to double-check this guide's information with it.
...
3. Connect the **Z-Motor** to port **ZAM**. You can also use port ZBM but may need to edit the printer.cfg.
## 4. Connecting the filament runout sensor
Try using [this reddit post](https://www.reddit.com/r/Ender3S1/comments/1685mug/ender_creality_filament_sensor_pinout/) to connect the sensor. If it doesn't work, try switching the signal and voltage :)
...
1. You'll need to remove a couple of the windings from the ferrite core for the cable to reach the port. I was left with only 2 windings for the cable length to be sufficient.  
...
1. Install Klipper on the RPi. If you've never done this before, the best way is using [KIAUH](https://github.com/dw-0/kiauh). That's what I personally use.
2. Use the **printer.cfg** from this guide's files as it has the right mappings for a BTT board inside an Ender-5 S1. Don't forget to change the mcu address and bltouch z_offset.
## 9. Mounting the RPi
1. You can use the mount from the description of [this YouTube video](https://www.youtube.com/watch?v=VSVQrhhNqhA).
2. Route all RPi cables through the cable hole on the back of the printer.
